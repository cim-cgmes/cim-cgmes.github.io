...
|-------|------|--------------|---------|
| RegulatingCondEq | [RegulatingCondEq](RegulatingCondEq.md) | 0..n | The equipment that participates in this regulating control scheme. |
| RegulationSchedule | [RegulationSchedule](RegulationSchedule.md) | 0..n | Schedule for this regulating control. |
| Terminal | [Terminal](Terminal.md) | 1..1 | The terminal associated with this regulating control. The terminal is associated instead of a node, since the terminal could connect into either a topological node or a connectivity node. Sometimes it is useful to model regulation at a terminal of a bus bar object. |
| discrete | Boolean | 1..1 | The regulation is performed in a discrete mode. This applies to equipment with discrete controls, e.g. tap changers and shunt compensators. |
| enabled | Boolean | 1..1 | The flag tells if regulation is enabled. |
| maxAllowedTargetValue | Float | 0..1 | Maximum allowed target value (RegulatingControl.targetValue). |
| minAllowedTargetValue | Float | 0..1 | Minimum allowed target value (RegulatingControl.targetValue). |
| mode | [RegulatingControlModeKind](RegulatingControlModeKind.md) | 1..1 | The regulating control mode presently available. This specification allows for determining the kind of regulation without need for obtaining the units from a schedule. |
| targetDeadband | Float | 0..1 | This is a deadband used with discrete control to avoid excessive update of controls like tap changers and shunt compensator banks while regulating. The units of those appropriate for the mode. The attribute shall be a positive value or zero. If RegulatingControl.discrete is set to 'false', the RegulatingControl.targetDeadband is to be ignored. Note that for instance, if the targetValue is 100 kV and the targetDeadband is 2 kV the range is from 99 to 101 kV. |
| targetValue | Float | 1..1 | The target value specified for case input. This value can be used for the target value without the use of schedules. The value has the units appropriate to the mode attribute. |
| targetValueUnitMultiplier | [UnitMultiplier](UnitMultiplier.md) | 1..1 | Specify the multiplier for used for the targetValue. |
